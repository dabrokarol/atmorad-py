...
def init_config():
    ...
    if out_path.exists():
        print(f"Error: '{out_path}' already exists. Delete if you want to reinitialize.", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("config", nargs="?", help="path to the simulation config TOML file", type=Path)
    parser.add_argument("--init", action="store_true", help="generate a default configuration file in the current directory")
